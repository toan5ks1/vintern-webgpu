...
async function genericTextToTextForward(self, {
    ...
    if (self.sessions['decoder_model_merged'].inputNames.includes('input_ids')) {
        const batch_size = inputs_embeds.dims[0];
        const seq_len = inputs_embeds.dims[1];
        decoder_inputs.input_ids = new Tensor('int64', new BigInt64Array(batch_size * seq_len).fill(0n), [batch_size, seq_len]);
    }

    const outputs = await decoderForward(self, decoder_inputs, true);
    return outputs;
}
...
export class InternVLChatModel extends LlavaPreTrainedModel {
    _merge_input_ids_with_image_features(kwargs) {
        const vision_hidden_size = kwargs.image_features.dims.at(-1);
        const reshaped_image_hidden_states = kwargs.image_features.view(-1, vision_hidden_size);
    ...
    static async from_pretrained(pretrained_model_name_or_path, options = {}) {
        options.subfolder ??= 'onnx';
        const config = await AutoConfig.from_pretrained(pretrained_model_name_or_path, options);
        const sessions = {
            embed_tokens: 'embed_tokens',
            vision_encoder: 'vision_encoder',
            decoder_model_merged: 'decoder_model_merged',
        };
        const [session_map, optional_configs] = await Promise.all([
            constructSessions(pretrained_model_name_or_path, sessions, { ...options, config }),
            getOptionalConfigs(pretrained_model_name_or_path, {
                generation_config: 'generation_config.json',
            }, options),
        ]);

        return new this(config, session_map, optional_configs);
    }

    async encode_image({ pixel_values }) {
        return (await sessionRun(this.sessions['vision_encoder'], { pixel_values })).last_hidden_state;
    }
}
//////////////////////////////////////////////////
...
export class SpeechT5ForTextToSpeech extends SpeechT5PreTrainedModel {
    ...
    async generate_speech(input_values, speaker_embeddings, {
        ...
        while (true) {
            ...
            console.log("Feeds keys:", Object.keys(decoderFeeds));
...
const MODEL_FOR_IMAGE_TEXT_TO_TEXT_MAPPING_NAMES = new Map([
    ...
    ['internvl_chat', ['InternVLChatModel', InternVLChatModel]],
]);
